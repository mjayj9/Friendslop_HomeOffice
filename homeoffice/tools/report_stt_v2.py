...
lines=['# 한국어 자막 실제 측정','', '**합성 음성 시험이며 실제 사람/마이크 품질 검증을 대신하지 않는다.**','',f"Whisper tiny q8 / Transformers.js 4.2.0 / ONNX WASM 1 thread, graph optimization disabled. 모델·설정·토크나이저 {out['modelBytes']:,} bytes. 한국어 6문장, Microsoft Heami Desktop, 1920×1080 Godot 동시 실행.",'',f"이 작은 합성 집합의 CER {out['cer']:.1%}, WER {out['wer']:.1%}. 이름·날짜·숫자 오류가 있어 품질 완료로 판정하지 않는다. 인식 후 LLM이나 규칙으로 의미를 추정하여 고치지 않았다.",'', 'CER: NFC/구두점 제거/공백 제외. WER: 같은 정규화 후 공백 단위. 숫자 표기는 강제 변환하지 않았다. 입력 구간 4.18–5.30초, 최종 배포 회귀 추론 1.29–1.50초. 실제 마이크의 발화 종료→최종 자막 지연과는 별도다.','', '| 원문 | 실제 인식 | 추론 ms | 게임 FPS 표본 |','|---|---|---:|---:|']
