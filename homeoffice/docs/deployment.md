...
이 브랜치의 `docs/`는 Godot 웹 export 정적 파일입니다. Pages의 브랜치 소스를 `codex/godot-homeoffice-v2`, 폴더를 `/docs`로 지정하면 예상 주소는 https://mjayj9.github.io/Friendslop_HomeOffice/ 입니다. **실제 배포/브라우저 확인 결과는 `evidence/pages-validation.json`을 확인하세요. 파일이 없으면 공개 검증을 완료했다고 뜻하지 않습니다.**
