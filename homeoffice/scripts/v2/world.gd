...
func _physics_process(dt):
	...
	if p:
		...
		var cmd={"x":float(Input.is_physical_key_pressed(KEY_D))-float(Input.is_physical_key_pressed(KEY_A)) if enabled else 0.0,"z":float(Input.is_physical_key_pressed(KEY_S))-float(Input.is_physical_key_pressed(KEY_W)) if enabled else 0.0,"yaw":yaw,"pitch":pitch,"jump":enabled and Input.is_physical_key_pressed(KEY_SPACE) and not p.command.jump,"run":enabled and Input.is_physical_key_pressed(KEY_SHIFT),"crouch":enabled and (Input.is_physical_key_pressed(KEY_C) or Input.is_physical_key_pressed(KEY_CTRL)),"seq":seq}
		p.command=cmd
...
func update_hint():
	...
	if p.seated!="":label="E 일어서기"+ (" · Z 취침" if p.posture=="lying" else "")
		...
		var labels={"chair":"E 앉기","sofa":"E 좌석에 앉기 · Z 눕기","bed":"E 눕기","table":"E 공동 보고서","fridge":"E 재료 꺼내기","cooker":"E 재료 조리 / 완성 음식 꺼내기","sink":"E 접시 정리","arcade":"E 아케이드 시작","counter":"E 음식 담기"}
...
func _unhandled_input(event):
	...
	if event is InputEventKey and event.pressed and not event.echo and playing():
		...
		if event.keycode==KEY_Z:request_action("eat" if players.has(local_id) and players[local_id].seated!="" and players[local_id].holding!="" and definitions[players[local_id].holding].kind=="plate" else "rest");return
		if event.keycode==KEY_R:request_action("secondary");return
		if event.keycode==KEY_B:
			if bridge:bridge.open_tool("catalog")
			return
		if event.keycode==KEY_F and players.has(local_id) and players[local_id].holding=="":request_action("kick_start");return
