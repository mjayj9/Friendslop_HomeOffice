...
func action(kind:String,data={}):
	order+=1
	w.perform("local",{"epoch":w.epoch,"seq":order,"action":kind,"data":data})
...
func run():
	w=load("res://scripts/v2/world.gd").new();root.add_child(w);await ticks(10);p=w.players.local;w.local_id="test-controller"
	at(Vector3(-13.8,0,-6.5),Vector3(-13.8,1,-8.6));action("interact")
	check("fridge gives one visible held ingredient",p.holding.begins_with("ingredient-"))
	at(Vector3(-11,0,-7),Vector3(-11,.7,-8.8));action("interact")
	...
	action("interact");await ticks(5)
	var food_id=""
	for id in w.objects:
		if w.definitions[id].kind=="meal":food_id=id
	check("cooked physical food appears",food_id!="")
	if food_id!="":
		at(Vector3(-11,0,-7),w.objects[food_id].position);action("interact");await ticks(5)
		check("cooked food can be picked up",p.holding==food_id)
		at(Vector3(-10.2,0,-4),Vector3(-10.2,.6,-5.7));action("interact")
		check("counter plates food as shared object state",w.objects["plate-1"].get_meta("state").get("food",false))
		at(Vector3(-9.8,0,-4.2),w.objects["plate-1"].position);action("interact");await ticks(3)
		check("plate can be carried",p.holding=="plate-1")
		at(Vector3(-10.38,0,2.5),Vector3(-10.38,.46,1.05));action("interact")
		...
		action("interact")
		at(Vector3(-7.8,0,-7),Vector3(-7.8,.7,-8.8));action("interact")
	...
	at(Vector3(-13.7,3.6,-8.6),Vector3(-13.7,4.1,-10.3));action("interact")
	...
	action("interact");check("wake finds safe standing location",p.seated=="" and p.posture=="standing")
	...
	check("basket downward crossing scores once",w.basketball_score==[2,0]);w.update_sports(1.0/60);check("duplicate basket sample does not score",w.basketball_score==[2,0])
	ball.position=Vector3(-16.35,3.12,30);ball.linear_velocity=Vector3(0,3,0);w.ball_previous.basketball=Vector3(-16.35,2.9,30);w.update_sports(1.0/60);check("upward crossing rejected",w.basketball_score==[2,0])
	at(Vector3(23,0,-2),w.objects["tag-gun-1"].position);action("interact");check("unapproved toy gun pickup rejected",p.holding=="")
	w.weapon_grants.local=true;action("interact");check("approved game-zone pickup succeeds",p.holding=="tag-gun-1")
	...
	var file=FileAccess.open("res://evidence/v2-interactions.json",FileAccess.WRITE);file.store_string(JSON.stringify({"environment":"Actual Godot/Jolt engine. Direct fixture positions and action requests; scoring uses controlled crossing samples. Not browser/manual validation.","results":results},"  "));file.close();quit(1 if results.any(func(r):return not r.passed) else 0)
