...
func run():
	...
	var file=FileAccess.open("res://evidence/v3/physics-lab.json",FileAccess.WRITE);file.store_string(JSON.stringify({"environment":"Godot 4.6.1 Jolt headless 60Hz, controlled independent physics scene, no user input simulation claim","results":results,"samples":lab.samples},"  "));file.close()
