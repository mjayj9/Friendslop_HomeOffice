...
func run():
	...
	var world=w.save_world();var file=FileAccess.open("res://evidence/v4/storage-world.json",FileAccess.WRITE);file.store_string(JSON.stringify(world));file.close()
