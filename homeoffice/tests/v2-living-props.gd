...
func action(p,kind):
	seq+=1;w.perform(p.actor_id,{"epoch":w.epoch,"seq":seq,"action":kind,"data":{}})
func run():
	...
	action(a,"interact");check("E opens actual hinged chest",box.get_meta("state").get("open",false))
	...
	action(a,"interact");w.update_living_props(.5)
	...
	action(a,"interact");w.update_living_props(1)
	var world=w.save_world();var file=FileAccess.open("res://evidence/v2-storage-world.json",FileAccess.WRITE);file.store_string(JSON.stringify(world));file.close()
	...
	action(a,"interact");action(b,"interact");w.update_living_props(.05)
	...
	print("DRAWER RAY ",w.target(a)," ",a.position," ",a.command);action(a,"interact");w.update_living_props(1);print("DRAWER ",drawer.get_meta("state")," ",drawer.get_node("MovingPart").position)
	...
	print("LAMP RAY ",w.target(a)," ",a.position," ",a.command);action(a,"interact");w.update_living_props(1);w.local_id="local";w.update_room_lighting(1);print("LAMP ",lamp.get_meta("state"))
	...
	file=FileAccess.open("res://evidence/v2-living-props.json",FileAccess.WRITE);file.store_string(JSON.stringify({"environment":"Actual Godot 4.6.1 / Jolt. Fixture positions and authentic target rays, action validation, meshes, physics and save/restore. Not human gameplay.","results":results},"  "));file.close();quit(1 if results.any(func(r):return not r.passed) else 0)
