...
func run():
	...
	var passed=bounces>=3 and height_min>.19 and height_max<2.0 and speed_max<=8.1 and p.holding=="basketball"
	...
	FileAccess.open("res://evidence/v2-dribble.json",FileAccess.WRITE).store_string(JSON.stringify(result,"  "))
	print("DRIBBLE ",result);quit(0 if passed else 1)
