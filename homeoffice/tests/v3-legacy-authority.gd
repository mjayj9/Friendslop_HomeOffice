...
func action(id:String,kind:String,data={}):
	seq+=1;w.perform(id,{"actor":id,"requestId":id+":"+str(seq),"targetId":"","epoch":w.epoch,"seq":seq,"action":kind,"data":data})
func run():
	...
	w.perform("local",{"actor":"local","requestId":"local:old-epoch","targetId":"","epoch":checkpoint.state.epoch,"seq":seq+100,"action":"interact","data":{}})
	...
	var f=FileAccess.open("res://evidence/v3/legacy-authority.json",FileAccess.WRITE);f.store_string(JSON.stringify({"environment":"Actual Godot/Jolt instance, fixture positions and action calls; not browser or human interaction", "results":results},"  "));f.close();var code=1 if results.any(func(r):return not r.passed) else 0;w.queue_free();w=null;a=null;b=null;chair=null;await process_frame;await create_timer(.25).timeout;quit(code)
