...
func run():
	...
	var f=FileAccess.open("res://evidence/v4/legacy-authority.json",FileAccess.WRITE);f.store_string(JSON.stringify({"environment":"Actual Godot/Jolt instance, fixture positions and action calls; not browser or human interaction", "results":results},"  "));f.close();var code=1 if results.any(func(r):return not r.passed) else 0;w.queue_free();w=null;a=null;b=null;chair=null;await process_frame;await create_timer(.25).timeout;quit(code)
