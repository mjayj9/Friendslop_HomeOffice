...
const MAX=32*1024*1024,assets=new Set(['chair','table','sofa','book','crate','marker','low_table','bed','counter','cooker','sink','fridge','plate','pan','basketball','football','ingredient','meal','gun','shield','arcade']);
...
function safeState(o){keys(o,[],['recipe','stage','food','dirty','cooking','ready','ammo','reloadUntil','nextShot','open','contents']);for(const[k,v]of Object.entries(o)){if(['recipe','stage'].includes(k)){if(!str(v,40))bad()}else if(['food','dirty','ready','open'].includes(k)){if(typeof v!=='boolean')bad()}else if(k==='contents'){if(!Array.isArray(v)||v.length>16||!v.every(x=>str(x)))bad()}else if(!finite(v,0,1e15))bad()}}
export function validateWorld(w){
 ...
 const ids=new Set();
 for(const o of w.objects){keys(o,['id','kind','p','yaw','state']);if(!str(o.id)||ids.has(o.id)||!assets.has(o.kind)||!Array.isArray(o.p)||o.p.length!==3||!o.p.every(x=>finite(x,-110,110))||!finite(o.yaw,-100000,100000))bad();safeState(o.state);ids.add(o.id)}
...
export async function encodeWorld(world,attachments=new Map()){
 ...
 for(const a of clean.presentation||[]){const b=attachments.get(a.id);if(!b)bad('발표 자료가 아직 수신되지 않았습니다');validAttachment(a,b);if(await sha(b)!==a.sha256)bad('자료 checksum 오류');entries.push(['attachments/'+a.id,b])}
...
export async function decodeBundle(bytes){
 ...
 for(const a of world.presentation||[]){const b=files.get('attachments/'+a.id);if(!b)bad('자료 누락');validAttachment(a,b);if(await sha(b)!==a.sha256)bad('자료 checksum 오류');attachments.set(a.id,b)}
