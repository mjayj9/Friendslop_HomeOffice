...
const broadcast=createBroadcast({identity:()=>id,isHost:()=>host,isClassroom:()=>classroomSession,players:()=>currentState?.players||[],authorized:who=>broadcastAllowed.has(who),send:(m,to='')=>net({...m,epoch},to),notify:status,onState:state=>push({type:"broadcast_visual",active:!!state.speaker}),onMode:mode=>{voiceMode=mode;voiceModes[id]=mode;$('voiceMode').value=mode;if(!host)net({type:'voice-settings',range,mode});}});
...
function bind(c){
 ...
 c.on('open',()=>{
  if(host&&c.metadata?.protocolVersion!==PROTOCOL_VERSION){c.send({type:'session-reject',reason:'서로 다른 게임 버전입니다. 양쪽 모두 새로고침하세요.'});setTimeout(()=>c.close(),400);return}
 ...
 c.on('data',m=>{
  ...
  if(host&&m.type==='voice-settings'){if(typeof m.range==='number'&&Number.isFinite(m.range))rangeByPeer[c.peer]=Math.min(voiceMax,Math.max(2,m.range));if(['near','zone','broadcast'].includes(m.mode))voiceModes[c.peer]=m.mode;return}
  ...
  if(!host&&m.type==='welcome'){if(m.roomCode)roomCode=m.roomCode;classroomSession=!!m.classroom;epoch=m.epoch;if(m.world.collaboration)collaboration.restore(m.world.collaboration);broadcast.restore(m.world.announcements||[]);documents.restore(m.world.documents||[]);entered();handoffSwitching=false;status('같은 집에 연결되었습니다. 서로의 캐릭터를 확인하세요.')}
...
async function startOnline(joinCode='',claimClassroom=false){
 broadcast.reset();broadcastAllowed.clear();voiceMode='near';voiceModes={};
 if(peer)return;
 let parsed;try{parsed=joinCode?parseInvitation(joinCode):parseInvitation(createRoomCode())}catch(e){status(e.message);return}
 roomCode=parsed.code;classroomSession=roomCode==='classroom';host=!joinCode||claimClassroom;id='ho3-user-'+crypto.randomUUID();epoch=crypto.randomUUID();
 ...
 peer.on('open',async()=>{
  try{
   ...
   if(host){room=id;push({type:'start',host:true,id,epoch,clientId:clientIdentity,classroom:classroomSession});entered();
    if(recoverPending){const w=recoverPending;recoverPending=null;collaboration.restore(w.collaboration);broadcast.restore(w.announcements||[]);documents.restore(w.documents||[]);arcade.restore({records:w.results.arcade||[]});push({type:'restore',world:w});}
   }else{const found=await resolveRoom(peer,roomCode);room=found.hostId;epoch=found.epoch;bind(peer.connect(room,{reliable:true,serialization:'binary',metadata:{clientId:clientIdentity,protocolVersion:PROTOCOL_VERSION}}));}
...
function showMenu(){push({type:"ui_focus",focused:true});menuVisible=true;playing=false;ptt=false;gateVoice();document.exitPointerLock?.();$('menu').hidden=false;push({type:'visibility',hidden:document.hidden});$('export').textContent=host?'공간 내보내기':'마지막 확인 복구본 내보내기';$('import').disabled=!host||connections.size>0}
function resume(){push({type:"activity",mode:""});broadcast.hide();documents.hide();collaboration.hide();activities.hide();presentation.hide();arcade.hide();menuVisible=false;playing=true;$('menu').hidden=true;$('tool').hidden=true;$('canvas').focus();push({type:'resume'})}
function openTool(mode){push({type:"ui_focus",focused:true});push({type:"activity",mode});playing=false;ptt=false;gateVoice();document.exitPointerLock?.();$('tool').hidden=false;broadcast.hide();documents.hide();collaboration.hide();activities.hide();presentation.hide();arcade.hide();$('book').hidden=mode!=='book';$('whiteboard').hidden=mode!=='board';$('boardTools').hidden=mode!=='board';$('toolTitle').textContent=({book:'잠깐, 책 한 권',board:'함께 그리는 보드',report:'함께 만드는 회의',catalog:'가구 놓기',runner:'작은 공룡 달리기',maze:'공룡 러너',documents:'회의 자료 · 컴퓨터',broadcast:'OFFICE 방송석'})[mode]||'함께 만드는 회의';if(mode==='broadcast')broadcast.open();else if(mode==='documents')documents.open();else if(mode==='presentation')presentation.open();else if(mode==='book')renderPage();else if(mode==='board')renderBoard();else if(['runner','maze'].includes(mode))arcade.open(mode);else if(mode==='catalog')activities.open(mode);else collaboration.open(mode)}
...
$('solo').onclick=()=>{broadcast.reset();directoryPeer?.destroy();directoryPeer=null;if(peer){peer.destroy();peer=null}connections.clear();host=true;id='local';epoch=crypto.randomUUID();room='';push({type:'start',host:true,id,epoch,clientId:clientIdentity});entered()};
...
function updateSpatial(){if(!context||!currentState)return;const me=currentState.players.find(p=>p.id===id);if(!me)return;for(const [who,v]of incoming){const p=currentState.players.find(p=>p.id===who);if(!p||!v.gain)continue;const dx=p.p[0]-me.p[0],dz=p.p[2]-me.p[2],d=Math.hypot(dx,dz),allowed=audible(who);v.active=allowed;const acoustic=currentState.voiceOcclusion?.[id+'|'+who]??(me.zone!==p.zone?0.22:1);const occluded=acoustic<1;const gain=allowed?((voiceModes[who]==='broadcast'&&broadcast.canHear(who,id))||voiceModes[who]==='zone'?1:Math.max(0,1-d/(rangeByPeer[who]||8)))*(voiceModes[who]==='broadcast'?1:acoustic)*(broadcast.active().speaker&&voiceModes[who]!=='broadcast'?0.35:1)*(gains.get(who)||0):0;v.gain.gain.setTargetAtTime(gain,context.currentTime,.08);v.filter.frequency.setTargetAtTime(occluded?1100:18000,context.currentTime,.1);v.panner.pan.setTargetAtTime(Math.max(-1,Math.min(1,(dx*Math.cos(me.yaw)-dz*Math.sin(me.yaw))/Math.max(d,1))),context.currentTime,.08)}}
...
window.Homeoffice={
 command_result:json=>{lastCommandResult=JSON.parse(json)},key_matches:(e,action)=>keyBindings.match(e,action),key_label:action=>keyBindings.label(action),
 avatar_diagnostics:json=>{avatarDiagnostics=JSON.parse(json)},physical_laser:(actor,uv)=>presentation.physicalPointer(actor,JSON.parse(uv)),gameplay_hud:json=>gameplayHud.update(JSON.parse(json),playing,currentState?.players.find(p=>p.id===id)?.zone),
 ...
 send(json,to){let m;try{m=JSON.parse(json)}catch{return}if(m.type==='welcome'){m.roomCode=roomCode;m.classroom=classroomSession;}if(m.world){m.world.announcements=broadcast.snapshot();m.world.documents=documents.snapshot();m.world.collaboration=collaboration.snapshot();m.world.presentation=presentation.metadata();m.world.presentationView=presentation.view();m.world.results.arcade=arcade.results();}net(m,to)},
...
const handoff=createHandoff({host:()=>host,id:()=>id,room:()=>room,peers:()=>[...connections.keys()],composing:()=>collaboration.diagnostics().composing,
 ...
 async switch(target,newEpoch,checkpoint){
  ...
  else{bind(peer.connect(room,{reliable:true,serialization:'binary',metadata:{clientId:clientIdentity,protocolVersion:PROTOCOL_VERSION}}));status('새 방장에게 다시 연결하는 중입니다.')}
 }
});
